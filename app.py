...
# Fix for the 'gpu_id' error: Force model to CPU
if hasattr(model, 'set_params'):
    model.set_params(gpu_id=-1, predictor='cpu_predictor')
...
def predict():
    try:
        ...
        # Support both nested and flat JSON
...
def home():
    return jsonify({"status": "Billing Anomaly API Active"})
